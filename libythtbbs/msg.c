...
translate_msg(char *src, struct msghead *head, char *dest, int add_site)
{
	...
	case 2:
		if (!head->sent) {
			sprintf(dest,
				...
				uid, time);
			sprintf(attstr, "\033[44;37;1m");
		} else {
			sprintf(dest,
				"\033[0;1;32m=>\033[37m%s\033[33m(%-16.16s)\033[37m                                              \033[m\n",
				uid, time);
	...
	case 5:
		if (!head->sent) {
			sprintf(dest,
				"\033[1;45m\033[36m%s\033[33m  (%-16.16s)\033[37m                                              \033[m\n",
				uid, time);
			sprintf(attstr, "\033[45;37;1m");
		} else {
			sprintf(dest,
				"\033[0;1;45;32m=>\033[37m%s\033[33m(%-16.16s)\033[37m                                              \033[m\n",
				uid, time);
			sprintf(attstr, "\033[45;36;1m");
		}
		break;
	case 0:
		sprintf(dest,
	...
	case 1:
		if (!head->sent) {
			...
			sprintf(dest,
				"\033[44m\033[37m你(%-16.16s) 邀请%-14.14s\033[37m                                           \033[m\n",
				time, head->id);
