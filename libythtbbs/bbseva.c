...
init_bbsevamsq()
{
	int msqid;
	struct msqid_ds buf;
	msqid = msgget(getBBSKey(BBSEVA_MSQ), IPC_CREAT | 0664);
	if (msqid < 0)
		return -1;
...
{
	char buf[256];
	snprintf(buf, sizeof (buf), "nONEID %s %s %s", board, filename, id);
...
bbseva_askavg(int ent, char *board, char *filename, float *avg)
{
	char buf[256];
	int retv;
	snprintf(buf, sizeof (buf), "nAVG %s %s", board, filename);
	if (bbseva_qry(ent, buf, sizeof (buf), 1) < 0)
		return -1;
...
{
	char buf[256];
	snprintf(buf, sizeof (buf), "nSET %s %s %s %d", board, filename, id,
		 star);
	    ...
	    int *oldstar, int *count, float *avg)
{
	char buf[256];
	int retv;
	snprintf(buf, sizeof (buf), "nQSET %s %s %s %d", board, filename, id,
		 star);
	if (bbseva_qry(ent, buf, sizeof (buf), 1) < 0)
		return -1;
