...
extern void init_echomailfp();
extern void init_bbsfeedsfp();
extern int isfile(char *file);
extern int isdir(char *file);
extern char *fileglue(char *fmt, ...);
extern int readfilterfile(char *);
extern int readNCMfile(char *);
extern int readnffile(char *inndhome);
extern int readnlfile(char *inndhome, char *outgoing);
extern int filtermatch(int result, char *target, char *pat);
extern FuncPtr search_filtercmd(char *cmd);
...
/* reopen bbslog */
	...
	sprintf(INNDTMPFS, "%s/bbstmpfs/innd", BBSHOME);
	...
	if (isfile(INNBBSCONF)) {
		...
		if (conf != NULL) {
			while (fgets(buffer, sizeof buffer, conf) != NULL) {
				char *ptr, *front = NULL, *value =
				    NULL, *value2 = NULL, *value3 = NULL;
				if (buffer[0] == '#' || buffer[0] == '\n')
					continue;
				for (front = buffer; *front && isspace(*front);
				     front++) ;
				for (ptr = front; *ptr && !isspace(*ptr);
				     ptr++) ;
				if (*ptr == '\0')
					continue;
				*ptr++ = '\0';
...
char *outgoing;
{
	...
	*NODELIST_BUF = '\0';
	...
	if (NODELIST == NULL) {
		...
		NODELIST_BYNODE =
		    (nodelist_t **) mymalloc(sizeof (nodelist_t *) *
					     (count + 1));
	} else {
		NODELIST =
...
char *inndhome;
{
	...
	*FILTER_BUF = '\0';
	...
	if (FILTERLIST == NULL) {
		FILTERLIST =
		    (filter_t *) mymalloc(sizeof (filter_t) * (count + 1));
	} else {
		FILTERLIST =
...
char *inndhome;
{
	...
	if (NEWSFEEDS_BUF == NULL) {
		...
		NEWSFEEDS_BUF =
		    (char *) myrealloc(NEWSFEEDS_BUF, st.st_size + 1);
	}
	*NEWSFEEDS_BUF = '\0';
	i = 0, count = 0;
	while (fgets(buff, sizeof buff, fp) != NULL) {
	...
	if (NEWSFEEDS == NULL) {
		NEWSFEEDS =
		    (newsfeeds_t *) mymalloc(sizeof (newsfeeds_t) *
					     (count + 1));
		NEWSFEEDS_BYBOARD =
		    (newsfeeds_t **) mymalloc(sizeof (newsfeeds_t *) *
					      (count + 1));
	...
	for (i = 0; i < NFCOUNT; i++) {
		...
		for (found = 0, ngptr = nfptr->newsgroups, ptr =
			...
			if (fptr) {
				...
				nfptr->rfilter =
				    (FuncPtr)
				    search_filtercmd(fptr->rcmdfilter);
				nfptr->sfilter =
				    (FuncPtr)
				    search_filtercmd(fptr->scmdfilter);
				found = 1;
			} else {
