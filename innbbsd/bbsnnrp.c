...
extern void verboseon(char *filename);
extern char *fileglue(char *,...);
extern void verboselog(char *, ...);
extern int unixclient(char *path, char *protocol);
...
void updaterc(char *actptr, int len, unsigned long value);
int myrename(char *old, char *new);
void flushrc(nnrp_t *bbsnnrp);
void writerc(nnrp_t *bbsnnrp);
char *NNRPgets(char *string, int len, FILE *fp);
int NNRPstat(nnrp_t *bbsnnrp, unsigned long artno, char **mid);
int NNRPxhdr(char *pattern, nnrp_t *bbsnnrp, int i, unsigned long low, unsigned long high);
int NNRPxhdrget(int *artno, char **mid, int iscontrol);
int INNBBSstat(nnrp_t *bbsnnrp, int i, char *mid);
int INNBBSihave(nnrp_t *bbsnnrp, unsigned long artno, char *mid);
int NNRPgroup(nnrp_t *bbsnnrp, int i, unsigned long *low, unsigned long *high);
void readnews(nnrp_t *bbsnnrp);
void INNBBSDhalt(void);
	...
	*DefaultTrustfrom = '\0';
	...
	while ((c = getopt(argc, argv, "f:F:m:o:g:w:r:p:a:s:t:h?ncv")) != -1)
		...
		case 'c':
			...
			break;
		case 'g':
			strncpy(DefaultNewsgroups, optarg,
				sizeof DefaultNewsgroups);
			break;
		case 'm':
			strncpy(DefaultModerator, optarg,
				sizeof DefaultModerator);
			break;
		case 'o':
			...
			break;
		case 'f':
			strncpy(DefaultTrustfrom, optarg,
				sizeof DefaultTrustfrom);
			break;
		case 'F':
			strncpy(DefaultTrustFrom, optarg,
				sizeof DefaultTrustFrom);
	...
	if (inputtype == NntpInputType) {
		...
		if (isfile(active)) {
			strncpy(BBSNNRP.activefile, active,
				sizeof BBSNNRP.activefile);
		...
		if ((lockfd = open(LockFile, O_RDWR | O_CREAT | O_EXCL, 0644)) <
		    0) {
		...
		for (;;) {
			...
			if (ptr != NULL)
				...
				ptr = active;
			sprintf(BBSNNRP.rcfile, "%s/.newsrc.%s.%s", INNDHOME,
				server, ptr);
			initrcfiles(&BBSNNRP);
		...
		while (fgets(buffer, sizeof buffer, stdin) != NULL) {
			...
			if (strncasecmp(buffer, "Message-ID: ", 12) == 0) {
				...
				approvedmet = 1;
				   ...
				   && *DefaultTrustfrom) {
				if (strstr(buffer + 6, DefaultTrustfrom) ==
				    NULL) {
					discard = 1;
				   ...
				   && *DefaultTrustFrom) {
				if (strstr(buffer + 5, DefaultTrustFrom) ==
				    NULL) {
					discard = 1;
					verboselog("Discard: %s for %s", buffer,
						   DefaultTrustFrom);
				...
				if (strncmp(buffer + 10, "from ", 5) == 0) {
					...
					if (rrptr != NULL)
						*rrptr = '\0';
				} else if (strncmp(buffer + 10, "(from ", 6) ==
					   0) {
					rptr += 6;
					rrptr = strchr(rptr, ')');
					savech = *rrptr;
...
nnrp_t *bbsnnrp;
{
	...
	for (ptr = bbsnnrp->actpointer;
       ...
       /**actlistptr = '\0';*/
		...
		if (*DefaultNewsgroups) {
			if (strncmp
			    (DefaultNewsgroups, rcptr->nameptr,
			     rcptr->namelen) != 0)
				continue;
...
nnrp_t *bbsnnrp;
{
...
#endif
	{
		...
		fprintf(stderr,
			"write to backup active fail. Maybe disk full\n");
...
char *mid;
{
	...
	if (artno != -1) {
		...
		if (atol(NNRPbuffer) != NNRParticleOK) {
			fputs(".\r\n", bbsnnrp->innbbsout);
			fflush(bbsnnrp->innbbsout);
			NNRPgets(INNBBSbuffer, sizeof INNBBSbuffer,
				 bbsnnrp->innbbsin);
			verboselog("innbbsGet: %s\n", INNBBSbuffer);
			return 0;
		}
...
nnrp_t *bbsnnrp;
{
	...
	for (i = 0; i < ACT_COUNT; i++) {
		...
		if (code == NNRPGroupOK) {
			...
			int isCancelControl =
			    (strncmp(rcptr->nameptr, "control", rcptr->namelen)
			     == 0)
...
/* less than or equal to high, for server renumber */
			if (rcptr->low != low) {
				bbsnnrp->actdirty = 1;
				rcptr->low = low;
				updaterc(rcptr->lowptr, rcptr->lowlen,
					 rcptr->low);
			}
			if (ResetActive) {
				if (rcptr->high != high) {
				...
				if (isCancelControl)
					xhdrcode =
					    NNRPxhdr("Control", bbsnnrp, i,
						     rcptr->high + 1, maxget);
				else
					xhdrcode =
					    NNRPxhdr("Message-ID", bbsnnrp, i,
						     rcptr->high + 1, maxget);
...
/*#define DEBUG*/
				...
				for (xcount = 0; xcount < artcount; xcount++) {
					...
					if (!ResetActive && mid != NULL)
						exception =
						    INNBBSihave(bbsnnrp, artno,
								mid);
					if (exception == -1)
						break;
					if (rcptr->high != artno) {
						rcptr->high = artno;
						updaterc(rcptr->highptr,
							 rcptr->highlen,
							 rcptr->high);
					}
				}
