...
int standaloneinit(char *port);
void innbbsusage(char *name);
int innbbsdstartup(void);
int main(int argc, char **argv);
extern int pmain(char *port);
extern void docompletehalt(int s);
extern int p_unix_main(char *path);
extern int INNBBSDshutdown();
extern void HISclose();
extern void HISmaint(); 
extern int tryaccept(int s); 
extern void verboselog(char *fmt, ...);
extern char *my_rfc931_name(int herefd, struct sockaddr_in *there);
extern int isPause();
extern int argify(char *line, char ***argvp);
extern daemoncmd_t *searchcmd(char *cmd); 
extern void deargify(char ***argv); 
extern void initial_lang();
extern void verboseon(char *filename);
extern void hisincore(int flag);
extern void startfrominetd(int flag);
extern int dbzdebug(int value);
extern void HISsetup();
extern void installinnbbsd();
extern void sethaltfunction(int (*haltfunc)());
	...
	if (client->ihavecount > 0 || client->statcount > 0) {
		bbslog
		    ("%s@%s rec: %d dup: %d fail: %d size: %d, stat rec: %d fail: %d, time sec: %d\n",
		     ...
		     client->ihaveduplicate, client->ihavefail,
		     client->ihavesize, client->statcount, client->statfail,
...
#ifdef DEBUG
				printf
				    ("before read i %d in.used %d in.left %d\n",
				     i, client[i].in.used, client[i].in.left);
#endif
				nr = channelreader(client + i);
#ifdef DEBUG
				printf
				    ("after read i %d in.used %d in.left %d\n",
				     i, client[i].in.used, client[i].in.left);
#endif
