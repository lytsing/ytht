...
nodelist_t *search_nodelist(char *site, char *identuser);
newsfeeds_t *search_group(char *newsgroup);
char *ascii_date(time_t now);
char *restrdup(char *ptr, char *string);
void *mymalloc(int size);
void *myrealloc(void *optr, int size);
void testandmkdir(char *dir);
char **split(char *line, char *pat);
char **BNGsplit(char *line);
char **ssplit(char *line, char *pat);
char *join(char **lineptr, char *pat, int num);
