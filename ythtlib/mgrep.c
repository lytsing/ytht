...
struct pattern_image {
	...
	unsigned int HASH[MAXHASH];  /* pattern hash入口，指向hashtable的位置 */
	...
	unsigned char pat_len[max_num]; /*每个pattern长度*/
	struct pat_list hashtable[max_num + 1]; /*所有pattern hash数据，0节点恒为index==0 */
};
