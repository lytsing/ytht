...
int unum;
{
	...
	if (suserc)
		sprintf(chatbuf + strlen(chatbuf),
			"[\033[36m%d\033[37m 人在高机密讨论室]", suserc);
...
char *msg;
{
	int i, utent = 0;	//, fd = users[unum].sockfd;
	char *utentstr;
	char *level;
	char *userid;
	char *chatid;
			...
			if (*msg == '%') {
				...
				if (isdigit(*msg) || ((c >= 'a') && (c <= 'f'))) {
					if ((c >= 'a') && (c <= 'f'))
						...
						color = c - '0';
					if (color > 7)
						sprintf(colorstr, "\033[1;%dm",
							color + 30);
					else
						sprintf(colorstr, "\033[%dm",
							color + 30);
...
/*    sprintf(chatbuf, "\033[1;36m%s \033[37m%s\033[m", users[unum].chatid, msg); */
		sprintf(chatbuf, "\033[1;36m%s \033[1;33m\033[m",
			users[unum].chatid);
		i = strlen(chatbuf);
		while ((*msg != 0) && (i < 252)) {
			if (*msg == '%') {
...
char *cmd;
{
	...
	for (i = 0; condition_data[i].verb; i++) {
		if (!strcmp(cmd, condition_data[i].verb)) {
			sprintf(chatbuf, "\033[1;36m%s \033[33m%s\033[37;0m",
				users[unum].chatid,
