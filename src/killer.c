...
void start_killergame()
{
    ...
    if(totalk>total) {
        send_msg(me, "\033[31;1m总人数少于要求的坏人人数,无法开始游戏\033[m");
        end_change_inroom();
        refreshit();
        return;
    }
    if(totalc==0)
        sprintf(buf, "\033[31;1m游戏开始啦! 人群中出现了%d个坏人\033[m", totalk);
    else
        sprintf(buf, "\033[31;1m游戏开始啦! 人群中出现了%d个坏人, %d个警察\033[m", totalk, totalc);
    send_msg(-1, buf);
    for(i=0;i<totalk;i++) {
        do{
            j=rand()%MAX_PEOPLE;
        ...
        send_msg(j, "用你的尖刀(\033[31;1mCtrl+S\033[m)选择你要残害的人吧...");
    }
    for(i=0;i<totalc;i++) {
        do{
            j=rand()%MAX_PEOPLE;
        ...
        send_msg(j, "白天请用你的警棒(\033[31;1mCtrl+T\033[m)选择你怀疑的人...");
    }
    for(i=0;i<MAX_PEOPLE;i++) 
    if(inrooms[myroom].peoples[i].style!=-1)
    if(!(inrooms[myroom].peoples[i].flag&PEOPLE_SPECTATOR))
        ...
        case '\r':
            switch(sel) {
                ...
                case 2:
                    ...
                    k_resetcolor();
                    clrtoeol();
                    k_getdata(t_lines-1, 0, "请输入名字:", buf, 13, 1, 0);
                    if(kicked) return 0;
                    if(buf[0]) {
                        k=1;
                ...
                case 4:
                    ...
                    k_resetcolor();
                    clrtoeol();
                    k_getdata(t_lines-1, 0, "请输入话题:", buf, 31, 1, 1);
                    if(kicked) return 0;
                    if(buf[0]) {
                        start_change_inroom();
                ...
                case 5:
                    ...
                    k_resetcolor();
                    clrtoeol();
                    k_getdata(t_lines-1, 0, "请输入房间最大人数:", buf, 30, 1, 1);
                    if(kicked) return 0;
                    if(buf[0]) {
                        i=atoi(buf);
                        if(i>0&&i<=100) {
                    ...
                    move(t_lines-1, 0);
                    clrtoeol();
                    k_getdata(t_lines-1, 0, "设置为隐藏房间?\033[m[Y/N]", buf, 30, 1, 1);
                    if(kicked) return 0;
                    buf[0]=toupper(buf[0]);
                    if(buf[0]=='Y'||buf[0]=='N') {
                    ...
                    move(t_lines-1, 0);
                    clrtoeol();
                    k_getdata(t_lines-1, 0, "设置为锁定房间?\033[m[Y/N]", buf, 30, 1,  1);
                    if(kicked) return 0;
                    buf[0]=toupper(buf[0]);
                    if(buf[0]=='Y'||buf[0]=='N') {
                    ...
                    move(t_lines-1, 0);
                    clrtoeol();
                    k_getdata(t_lines-1, 0, "设置为拒绝旁观者的房间?\033[m[Y/N]", buf, 30, 1, 1);
                    if(kicked) return 0;
                    buf[0]=toupper(buf[0]);
                    if(buf[0]=='Y'||buf[0]=='N') {
                    ...
                    move(t_lines-1, 0);
                    clrtoeol();
                    k_getdata(t_lines-1, 0, "设置坏人的数目:", buf, 30, 1,  1);
                    if(kicked) return 0;
                    if(buf[0]) {
                        i=atoi(buf);
                        if(i>=0&&i<=10) {
                    ...
                    move(t_lines-1, 0);
                    clrtoeol();
                    k_getdata(t_lines-1, 0, "设置警察的数目:", buf, 30, 1,  1);
                    if(kicked) return 0;
                    if(buf[0]) {
                        i=atoi(buf);
                        if(i>=0&&i<=2) {
                ...
                case 6:
                    ...
                    k_resetcolor();
                    clrtoeol();
                    k_getdata(t_lines-1, 0, "请输入要踢的id:", buf, 30, 1,  1);
                    if(kicked) return 0;
                    if(buf[0]) {
                        for(i=0;i<MAX_PEOPLE;i++)
                            if(inrooms[myroom].peoples[i].style!=-1)
            ...
            if(ch==KEY_UP) {
                ...
                if(inrooms[myroom].peoples[me].flag & PEOPLE_ALIVE &&
                    ...
                    if(inrooms[myroom].peoples[sel].flag&PEOPLE_ALIVE && 
                        ...
                        sel!=me) {
                        int i,j,t1,t2,t3;
                        sprintf(buf, "\033[32;1m%d %s投了%d %s一票\033[m", me+1, inrooms[myroom].peoples[me].nick,
   				    ...
   				    //save_result(1);
                                    ...
                                    for(i=0;i<MAX_PEOPLE;i++) 
                                        ...
                                        if(inrooms[myroom].peoples[i].flag&PEOPLE_KILLER&&inrooms[myroom].peoples[i].flag&PEOPLE_ALIVE)
                                            send_msg(i, "请抓紧你的宝贵时间用\033[31;1mCtrl+S\033[m杀人...");
                                }
                                else {
                                    for(i=0;i<MAX_PEOPLE;i++)
                        ...
                        if(sel==me)
                            send_msg(me, "\033[31;1m你不能选择自杀\033[m");
                        else if(!(inrooms[myroom].peoples[sel].flag&PEOPLE_ALIVE))
                            send_msg(me, "\033[31;1m此人已死\033[m");
                        else
                            send_msg(me, "\033[31;1m此人是旁观者\033[m");
        ...
        if(msg[0]=='/'&&msg[1]=='/') {
            ...
            for(i=0;;i++) {
                ...
                if(!strcasecmp(party_data[i].verb, buf)) {
                    k=0;
                    sprintf(buf, "%s \033[1;37m%s\033[0m %s", party_data[i].part1_msg, buf2[0]?buf2:"大家", party_data[i].part2_msg);
                    break;
                }
            }
            if(k)
...
quitgame:
    ...
    if(inrooms[myroom].peoples[me].flag&PEOPLE_KILLER)
        sprintf(buf, "\033[34m杀手%s潜逃了...\033[0m", inrooms[myroom].peoples[me].id);
    else
        sprintf(buf, "\033[36m%s离开房间\033[0m", inrooms[myroom].peoples[me].id);
    send_msg(-1, buf);
quitgame2:
    kicked=0;
...
static int room_list_refresh(struct _select_def *conf)
{
    ...
    move(2, 0);
    prints("\033[0;1;37;44m    %4s %-14s %-12s %4s %4s %6s %-20s\033[m", "编号", "游戏室名称", "创建者", "人数", "最多", "锁隐旁", "话题");
    clrtoeol();
    k_resetcolor();
    update_endline();
