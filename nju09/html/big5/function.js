...
function checkFrame()
{
	...
	document.writeln("<a href=bbsindex?t=1&b="+d.substring(d.lastIndexOf('/')+1,d.length)+" class=blu>顯示完整界面</a> ");
}
function openlog()
{
...
var s;
	...
	document.writeln("喜歡這個文章麼? 這個文章"+s+"1'>不錯</a>]"+s+"3'>很好</a>]"+s+"5'>頂一下!</a>]");
}
function docform(a, b){
	document.writeln("<table border=0 cellspacing=0 cellpading=0><tr><td><form name=docform1 action="+a+"><a href="+a+"?B="+b+"&S=1>第一頁</a> <a href="+a+"?B="+b+"&S=-100>最後一頁</a> <input type=hidden name=B value="+b+"><input type=submit value=轉到>第<input type=text name=start size=4>篇</form></td><td><form name=docform2 action="+a+"><input type=submit value=轉到><input type=text name=B size=7>討論區</form></td></tr></table>");
}
//tabs顯示版面選項卡
...
function printThreadDocPages(url, page, total)
{
	//total = 50;
	var url2=url+"&P=";
	var s="<div class=pages><table><tr><td><nobr>第"+page+"頁 共"+total+"頁</nobr></td><td>";
	...
	if(page>1) s+=p1+"<a href="+url2+(page-1)+">&lt;上頁</a>";
	...
	if(page<total)
		s+=p1+"<nobr><a href="+url2+(page+1)+">下頁&gt;</a></nobr>";
	if(end<total)
		s+=p1+"<nobr><a href="+url2+0+">最後<b>&raquo;</b></a></nobr>";
...
lm2str(bl)
{
	...
	if(bl.isclose==1)
		return '<font class=gry>封閉俱樂部</font>';
	if(bl.isclose==2)
		retv+='<b>本版只讀</b><br>';
...
oneBoard(bl)
{
	...
	retv+='<tr><td valign=top width=160><a name='+bl.board+'></a>'
	 ...
	 +'</a></center>'+a
	 +'<font class=f1>人氣﹕'+bl.score+'</font> &nbsp;'
	 +'<font class=f1>文章數﹕'+bl.total+'</font><br>'
...
printMypic(magic,user,t,npost,life,experience,experienceStr,perf,perfStr,hasblog)
{
	...
	+'文章數﹕'+npost
	+'<br>生命﹕'+life
	+'<br>經驗﹕'+experience+'('+experienceStr+')'
	+'<br>表現﹕'+perf+'('+perfStr+')'
	+blog
	+'</font></div>');
}
