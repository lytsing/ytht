...
//      static int r = 0;
	...
	if (!loginok || isguest) {
		printf
		    ("<body topmargin=1 MARGINHEIGHT=1><script>"
		     "top.document.getElementById('fmsg').style.height='2px';top.doResize();"
		     "</script>\n</html>\n");
		return 0;
	}
	if (u_info->unreadmsg > 0) {
		...
		printf
		    ("<script>top.document.getElementById('fmsg').style.height=\"%dpx\";"
		     "top.doResize();</script>\n", (line + 11) * 16 + 32);
		emotion_print(msg);
		if (head.mode == 0) {
	...
	printf
	    ("<body topmargin=1 MARGINHEIGHT=1><script>"
	     "top.document.getElementById('fmsg').style.height=\"2px\";top.doResize();</script>");
...
check_msg()
{
	if (loginok && !isguest && u_info->unreadmsg > 0)
		printf("<script>if(!top.fmsg.form0)"
		       "top.fmsg.location.replace(\"bbsgetmsg?sn=%ld\");</script>\n",
		       now_t);
}
