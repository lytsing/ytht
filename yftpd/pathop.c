...
constructpath(const char *fname)
{
	...
	if (*fname == '/') {
		...
		if (strlen(cwd) + strlen(fname) + 1 >= sizeof (partpath))
			return NULL;
		if (strncmp(cwd, ftpRealRoot, len))
	...
	if (strlen(ftpRealRoot) + strlen(partpath) >= sizeof (fullpath))
		return NULL;
	strcpy(fullpath, ftpRealRoot);
...
my_getcwd(char *buf, size_t size)
{
	...
	if (!(retv = getcwd(buf, size)))
		return NULL;
	if (!strcmp(buf, ftpRealRoot)) {
		strcpy(buf, "/");
		return buf;
	}
	memmove(buf, buf + strlen(ftpRealRoot),
		strlen(buf) - strlen(ftpRealRoot) + 1);
	return buf;
}
