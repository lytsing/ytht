...
do_stor(char *filename, int flags)
{
	...
	if (quota->q[quota_type] <= 0) {
		//yftpd_log ("Error: quota limits reached while trying to store file '%s'.\n", filename);
...
command_mkd(char *dirname)
{
	if (my_mkdir(dirname, 0755)) {
		//yftpd_log ("Error: '%s' while trying to create directory '%s'.\n", strerror(errno), dirname);
		prints("451 Error: %s.", strerror(errno));
	} else {
...
{
	if (my_rmdir(dirname)) {
		//yftpd_log ("Error: '%s' while trying to remove directory '%s'.\n", strerror(errno), dirname);
		prints("451 Error: %s.", strerror(errno));
	} else {
...
{
	FILE *fp;
	if ((fp = my_fopen(oldname, "r"))) {
		fclose(fp);
		mystrncpy(filename, oldname, sizeof (filename) - 1);
		state = STATE_RENAME;
...
{
	if (my_rename(filename, newname)) {
		//yftpd_log("Error: '%s' while trying to rename '%s' to '%s'.\n", strerror(errno), filename, newname);
		prints("451 Error: %s.", strerror(errno));
	} else {
...
command_adminquota(char *params)
{
	...
	for (i = 0; i < NUM_QUOTA_TYPE; i++)
		prints("200-%-7d %-7dM %s", i, (int) (quota->q[i] / 1024 / 1024),
		       quota->d[i]);
...
command_adminsetquota(char *params)
{
	...
	if (2 != sscanf(params, "%d %d", &quotaid, &sizeM)) {
		prints
		    ("200 Sytax error: setquota <quotaid> <newsize in Mbytes>.");
		return;
	}
	...
	if ((long long)sizeM * 1024 *1024 < 0) {
		prints("200 Invalid quota size.");
		return;
	}
	quota->q[quotaid] = (long long)sizeM * 1024 *1024;
	do_savequota(1);
	prints("200 quota size of %d is set to %d Mbytes.", quotaid,
	       sizeM);
}
...
const struct command commands[] = {
	...
	{"TYPE", "<sp> type-code (A or I)", command_type, STATE_AUTHENTICATED,
	 0},
	{"PORT", "<sp> h1,h2,h3,h4,p1,p2", command_port, STATE_AUTHENTICATED,
	 0},
	...
	{"QUIT", "(close control connection)", command_quit, STATE_CONNECTED,
	 0},
...
parsecmd(char *str)
{
	...
	*pp++ = 0;
	for (i = 0; commands[i].name; i++) {	/* Parse command */
		if (strncasecmp
		    (str, commands[i].name, strlen(commands[i].name)))
			continue;
