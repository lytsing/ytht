...
show_issue()
{
	...
	prints
	    ("[20;1H[1;33mftp://dii.nju.edu.cn/pub/bbs/dosmine.exe [m»¹ÓÐ±¾ÓÎÏ·µÄWindowsÊó±ê°æ, »¶Ó­ÏÂÔØ.\r\n");
	refresh();
	pressanykey();
}
...
winloop()
{
	...
	while (1) {
		...
		if (c == 259 && x < 30)
			x++;
		move(0, 20);
		sprintf(buf, "Ê±¼ä: %d ", (int) (time(0) - t0));
		prints(buf);
		move(40, 20);
...
winrefresh()
{
	...
	move(0, 23);
	prints
	    ("[1;32mÉ¨À×[mfor bbs v1.00 by zhch.nju 00.3, press '[1;32mh[m' to get help, '[1;32m^C[m') to exit.");
	for (y = 1; y <= 16; y++) {
		move(0, y - 1);
		for (x = 1; x <= 30; x++)
			winsh0(x, y);
...
winhelp()
{
	clear();
	prints
	    ("==»¶Ó­À´[1;35m" MY_BBS_NAME
	     "[mÍæ¼üÅÌÉ¨À×ÓÎÏ·==\r\n---------------------------------\\r\n\r\n");
	prints("Íæ·¨ºÜ¼òµ¥£¬ºÍ[1;34mwindows[mÏÂµÄÊó±êÉ¨À×²î²»¶à.\r\n");
	prints
	    ("  '[1;32mA[m'¼üµÄ×÷ÓÃÏàµ±ÓÚÊó±êµÄ×ó¼ü¼°Ë«»÷µÄ×÷ÓÃ£¬ ³ÌÐò¸ù¾ÝÄãµã»÷µÄÎ»ÖÃ\r\n");
	...
	prints
	    ("½¨ÒéÓÃ[1;32mNetterm[mÀ´Íæ(µ±È»njutermÒ²¿ÉÒÔ,:)),[1;32mtelnet[mÐ§¹û²»ÊÇÌ«ºÃ\r\n");
...
win_showrec()
{
	...
	clear();
	prints
	    ("[44;37m                      -" MY_BBS_NAME " BBS ¸ÐÓ¦Ê½É¨À×ÅÅÐÐ°ñ-                             \r\n[m");
	prints
	    ("[41m Ãû´Î       Ãû×Ö        ºÄÊ±                      À´×Ô                     ³é½±[m\r\n");
	for (n = 0; n <= 19; n++) {
