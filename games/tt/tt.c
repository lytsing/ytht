...
tt_game()
{
	...
	//report("tt game");
	tt_load_record();
        printf("\033[2J\033[44;37m                       -" MY_BBS_NAME " BBS ´ò×Ö¸ßÊÖÅÅÐÐ°ñ-                             \r\n\033[m");
	printf("\033[41m Ãû´Î       Ãû×Ö        ËÙ¶È(WPM)                 À´×Ô                     ³é½±\033[m\r\n");
	for (n = 0; n <= 19; n++) {
...
//		       tt_ip[n], tt_scr[n] / 10.);
	...
	read(0, genbuf, 32);
	printf
	    ("[2J[1;32m£Â£Â£Ó[m´ò×ÖÁ·Ï°³ÌÐò. (´óÐ¡Ð´¾ù¿É, ÊäÈëµÚÒ»×Ö·ûÇ°µÄµÈ´ý²»¼ÆÊ±. [1;32m^C[m or [1;32m^D[m ÍË³ö.)\r\n\r\n\r\n");
	...
	if (tt_check_record(score))
		printf("[1;33m×£ºØ£¡ÄúË¢ÐÂÁË×Ô¼ºµÄ¼ÍÂ¼£¡[m\r\n\r\n");
	fflush(stdout);
	goto start;
}
