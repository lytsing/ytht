...
#include <sys/mman.h>
#include "bbs.h"
#define DICTPATH MY_BBS_HOME "/etc/ncce"

char *
nccemmapfile(char *fn)
{
	char *ptr;
	int fd;
	struct stat st;
...
search_dict(char *word)
{
	...
	if (((unsigned char) *word) < 128) {
		if (ecidx == NULL) {
			ecidx = nccemmapfile(DICTPATH "/EC.IDX");
			eclib = nccemmapfile(DICTPATH "/EC.LIB");
		}
		idx = ecidx;
		lib = eclib;
	} else {
		if (ceidx == NULL) {
			ceidx = nccemmapfile(DICTPATH "/CE.IDX");
			celib = nccemmapfile(DICTPATH "/CE.LIB");
		}
		idx = ceidx;
		lib = celib;
	}
