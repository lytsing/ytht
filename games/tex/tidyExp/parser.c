        ...
        if (!(node->tag->model & CM_INLINE))
        {
            ...
            if (element->tag == tag_td || element->tag == tag_th)
            {
                ...
                if (node->tag->model & CM_LIST)
                {
                    ...
                    AddAttribute(node, "style", "margin-left: -2em");
    ...
    */
    ...
    while ((node = GetToken(lexer, mode)) != null)
    {
        ...
        if (node->type == StartTag
                ...
                && node->tag != tag_font)
        {
            if (node->attributes == null && node->tag == element->tag)
            {
...
Node *ParseXMLDocument(Lexer *lexer)
{
    ...
    /* XXX For MZ: fix stylesheets */
    if (ForMZ)
        FixXMLStyleSheets(lexer, document);
